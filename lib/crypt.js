...
function encrypt(data, key) {
    ...
    let aes = createCipheriv('aes-256-cbc', Buffer.from(salted.substring(0, 64), 'hex'), // Key
    Buffer.from(salted.substring(64, 64 + 32), 'hex'));
...
function decrypt(rawData, key) {
    ...
    let result = arr[0];
    for (let x = 1; x < 3; x++) {
        arr.push(Buffer.from(createHash('md5')
            .update(Buffer.concat([Buffer.from(arr[x - 1], 'hex'), dk]))
            .digest()).toString('hex'));
        result += arr[x];
    }
    let aes = createDecipheriv('aes-256-cbc', Buffer.from(result.substring(0, 64), 'hex'), Buffer.from(data.iv, 'hex'));
