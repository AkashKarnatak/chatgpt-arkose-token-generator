...
function cfpHash(H8W: any) {
  if (!H8W) return ''
  return H8W.split('').reduce(function (p8W: any, z8W: any) {
    p8W = (p8W << 5) - p8W + z8W.charCodeAt(0)
    return p8W & p8W
  }, 0)
}

function prepareF(fingerprint: any) {
  let f: any[] = []
...
function prepareFe(fingerprint: any) {
  let fe: string[] = []
  let keys = Object.keys(fingerprint)
  for (let i = 0; i < keys.length; i++) {
    switch (keys[i]) {
...
const baseEnhancedFingerprint = {
  ...
  audio_codecs:
    '{"ogg":"probably","mp3":"probably","wav":"probably","m4a":"maybe","aac":"probably"}',
  video_codecs:
    '{"ogg":"probably","h264":"probably","webm":"probably","mpeg4v":"","mpeg4a":"","theora":""}',
  ...
  window__location_href:
    'https://roblox-api.arkoselabs.com/v2/1.5.5/enforcement.fbfc14b0d793c6ef8359e0e4b4a91f67.html#476068BF-9607-4799-B53D-966BE98E2B81',
...
function getEnhancedFingerprint(
  fp: typeof baseFingerprint,
  ua: string,
  opts: any,
) {
  let fingerprint = { ...baseEnhancedFingerprint }

  fingerprint.webgl_extensions = fingerprint.webgl_extensions
    .split(';')
    .filter((_) => Math.random() > 0.5)
    .join(';')
  fingerprint.webgl_extensions_hash = x64hash128(
    fingerprint.webgl_extensions,
    0,
  )
  ...
  fingerprint.webgl_hash_webgl = x64hash128(
    Object.entries(fingerprint)
      .filter(([k, _]) => k.startsWith('webgl_') && k != 'webgl_hash_webgl')
      .map(([_, v]) => v)
      .join(','),
    0,
  ) as any
  ...
  fingerprint.client_config__surl =
    opts.surl || 'https://client-api.arkoselabs.com'
